...
class EneoTokenServiceTests {
	...
	@Mock
	private ResponseEntity<EneoTokenService.AccessTokenResponse> responseEntityMock;

	private EneoTokenService tokenService;

	@BeforeEach
	void setUp() {
		tokenService = new EneoTokenService(restClientMock, "someUsername", "somePassword");
	}

	@Test
	void getToken() {
		// Create a (non-expired) token
		final var accessToken = createToken(Instant.now().plus(1, ChronoUnit.DAYS));

		when(responseEntityMock.getBody()).thenReturn(new EneoTokenService.AccessTokenResponse(accessToken, "bearer"));
		...
		when(responseSpecMock.toEntity(EneoTokenService.AccessTokenResponse.class)).thenReturn(responseEntityMock);
		...
		verify(responseSpecMock).toEntity(EneoTokenService.AccessTokenResponse.class);
	...
	void getTokenWhenTokenIsExpired() {
		...
		final var accessToken = createToken(Instant.now().minus(1, ChronoUnit.HOURS));
		...
		verify(responseSpecMock, times(2)).toEntity(EneoTokenService.AccessTokenResponse.class);
	...
	void accessTokenResponseCreationAndAccessors() {
		final var accessToken = "someAccessToken";
		final var tokenType = "someTokenType";

		final var accessTokenResponse = new EneoTokenService.AccessTokenResponse(accessToken, tokenType);
